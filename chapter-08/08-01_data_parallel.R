...
library(tm)
data("acq")
textdata <- rep(sapply(content(acq), function(x) content(x)),
                1e5)
...
system.time(res2 <- unlist(
    parSapply(cl, text.partitioned,
              function(text, pattern) grepl(pattern, text),
              pattern)
)) 
stopCluster(cl)
...
system.time(res3 <- unlist(
    mclapply(text.partitioned,
             function(text, pattern) grepl(pattern, text),
             pattern, mc.cores = detectCores())
))
